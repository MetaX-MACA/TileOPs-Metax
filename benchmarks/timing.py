...
def _collect_torch_profiler(
    ...
    """Collect kernel timings through PyTorch's Kineto/CUPTI integration.
...
def _reset_persisting_l2_cache() -> None:
    global _cuda_runtime, _cuda_runtime_unavailable
    if _cuda_runtime_unavailable:
        return
    if _cuda_runtime is None:
        try:
            from cuda.bindings import runtime as cuda_runtime
        except ModuleNotFoundError:
            # cuda-python is NVIDIA-specific. MACA uses torch.cuda-compatible
            # tensors but does not provide cuda.bindings; the buffer flush below
            # remains available as the portable cache-isolation fallback.
            _cuda_runtime_unavailable = True
            _logger.info(
                "cuda.bindings is unavailable; skipping persisting-L2 reset "
                "and using the portable flush buffer"
            )
            return
