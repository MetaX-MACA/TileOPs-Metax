    ...
    PARAMS = [
        ("m, n, k, dtype, trans_a, trans_b, tune", [
            pytest.param(
                ...
                marks=[pytest.mark.smoke, pytest.mark.packaging, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")],
                id="smoke-fp16-square",
            ),
            pytest.param(
                1024, 1024, 1024, torch.bfloat16, False, False, False,
                marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")],
...
class GemvBoundaryFixture(FixtureBase):
    ...
    PARAMS = [
        ("n, k, dtype, tune", [
            ...
            pytest.param(3000, 1024, torch.float16, False, marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")]),
            pytest.param(3000, 1024, torch.bfloat16, False, marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")]),
