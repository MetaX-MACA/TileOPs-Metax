...
class DeltaNetBwdFixture(FixtureBase):
    PARAMS = [
        ("batch, seq_len, heads, dim_k, dim_v, chunk_size, dtype, tune", [
            pytest.param(2, 64, 2, 64, 64, 32, torch.float32, False, marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")]),
            pytest.param(2, 64, 2, 64, 64, 32, torch.float16, False, marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")]),
            pytest.param(2, 64, 2, 64, 64, 32, torch.bfloat16, False, marks=[pytest.mark.smoke, pytest.mark.xfail(reason="MACALaunch mcErrorInvalidValue")]),
