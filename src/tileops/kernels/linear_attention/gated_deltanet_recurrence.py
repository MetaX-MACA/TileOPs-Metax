    ...
    def _decode_func(num_stages, threads=128):
        ...
        def _decode_body(
            ...
            with T.Kernel(batch, head, threads=threads) as (bid, hid):
                ...
                for j in T.Parallel(dim_v):
                    v_new[j] = (
                        beta_val * T.cast(v[bid, hid, j], accum_dtype) - alpha_beta * sk_frag[j]
                    )

                # o = alpha * (S @ q) + (q . k) * v_new
                for j in T.Parallel(dim_v):
                    o[bid, hid, j] = T.cast(alpha * sq_frag[j] + qk_dot[0] * v_new[j], dtype)
            ...
            for num_stages in [1, 2, 3]:
                for threads in [128, 256]:
                    try:
                        ...
                        t = do_bench(lambda _fn=fn: _fn(q, k, v, g, beta, state), warmup=10, rep=20)
                        if t < best_time:
                            best_time = t
                            best_config = {
                ...
                for kt in T.Pipelined(dim_k // k_tile, num_stages=num_stages):
                    ...
                    for kk, j in T.Parallel(k_tile, dim_v):
                        new_state[bid, hid, kt * k_tile + kk, j] = (
                            alpha * h_tile[kk, j] + k[bid, hid, kt * k_tile + kk] * v_new[j]
                        )
